function [bRet, charBuffer] = qKLT(c, szStartCoordSystem, szEndCoordSystem)
% function bRet = qKLT(c, szNameOfCoordSystem, szAxes, ValueArray)
...
while (bufferOverflow)
    try
    [bRet, ~, ~, charBuffer] = calllib(c.libalias, FunctionName, c.ID, szStartCoordSystem, szEndCoordSystem, charBuffer, bufsize);
        if(bRet==0)
            iError = GetError(c);
