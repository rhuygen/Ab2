function [bRet, charBuffer] = qKLC (c, szNameOfCoordSystem, szNameOfCoordSystem2, szItem1, szItem2)
% function bRet = qKLC (c, szNameOfCoordSystem, szAxes, ValueArray)
...
while (bufferOverflow)
    try
        [bRet, ~, ~, ~, ~, charBuffer] = calllib (c.libalias, FunctionName, c.ID, szNameOfCoordSystem, szNameOfCoordSystem2, szItem1, szItem2, charBuffer, bufsize);
        if (bRet==0)
            iError = GetError (c);
