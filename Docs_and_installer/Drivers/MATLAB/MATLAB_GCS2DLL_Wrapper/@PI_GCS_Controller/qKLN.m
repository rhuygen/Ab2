function [bRet, charBuffer] = qKLN (c, szNamesOfCoordSystems)
% function bRet = qKLN (c, szNameOfCoordSystem, szAxes, ValueArray)
...
while (bufferOverflow)
    try
        [bRet, ~, charBuffer] = calllib (c.libalias, FunctionName, c.ID, szNamesOfCoordSystems, charBuffer, bufsize);
        if (bRet==0)
            iError = GetError (c);
