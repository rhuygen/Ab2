function [bRet, charBuffer] = qKET(c, szTypes)
% function bRet = qKET(c, szNameOfCoordSystem, szAxes, ValueArray)
...
while (bufferOverflow)
    try
    [bRet, ~, charBuffer] = calllib(c.libalias, FunctionName, c.ID, szTypes, charBuffer, bufsize);
        if(bRet==0)
            iError = GetError(c);
